...
// markDirtyXY is the hot-path variant for callers that already know the
// buffer coordinates.
func (b *Buffer) markDirtyXY(i, x, y int) {
...
func (b *Buffer) Set(x, y int, ch rune, st style.Style) {
	...
	b.setRuneCell(x, y, ch, st)
}

// setASCIICell is the common narrow-character path used by SetString. ASCII
// source/UI text cannot itself create a wide glyph, so avoid the full width
// classification on every cell. We still repair an existing wide glyph when
// an ASCII write lands on its continuation/leading cell.
func (b *Buffer) setASCIICell(x, y int, ch rune, st style.Style) {
	i := b.idx(x, y)
	if b.back[i].Ch == 0 && x > 0 {
		prev := i - 1
		if b.back[prev].Ch != 0 && RuneWidth(b.back[prev].Ch) == 2 {
			b.back[prev] = blank
			b.markDirtyXY(prev, x-1, y)
		}
	}
	...
	if x+1 < b.W && b.back[i+1].Ch == 0 && b.back[i+1] != blank {
		b.back[i+1] = blank
		b.markDirtyXY(i+1, x+1, y)
	}
}

func (b *Buffer) setRuneCell(x, y int, ch rune, st style.Style) {
	i := b.idx(x, y)
	// Writing into the continuation half of a wide glyph must first remove the
	// glyph's leading cell. Otherwise a clipped redraw can leave a stale wide
	// character behind its newly written cell.
	if x > 0 {
		prev := i - 1
		if b.back[prev].Ch != 0 && RuneWidth(b.back[prev].Ch) == 2 && b.back[i].Ch == 0 {
			b.back[prev] = blank
			b.markDirtyIdx(prev)
		}
	}
	...
	w := RuneWidth(ch)
	// Replacing the leading half of an old wide glyph with a narrow glyph must
	// clear its continuation cell.
	if w != 2 && x+1 < b.W {
		ci := i + 1
		if b.back[ci].Ch == 0 && RuneWidth(b.back[i].Ch) != 2 {
			// The new leading cell is narrow, so any continuation here is stale.
			cont := blank
			if b.back[ci] != cont {
				b.back[ci] = cont
				b.markDirtyIdx(ci)
	...
	if w == 2 && x+1 < b.W && b.InBounds(x+1, y) {
		ci := b.idx(x+1, y)
		cont := Cell{Ch: 0, Style: st}
		if b.back[ci] != cont {
			b.back[ci] = cont
			b.markDirtyIdx(ci)
		}
...
// SetCell writes a pre-composed terminal cell directly. It is intended for
// widgets that already own a coherent cell grid, such as the VT emulator. It
// skips wide-glyph repair and RuneWidth classification while retaining normal
// clipping and dirty tracking.
func (b *Buffer) SetCell(x, y int, ch rune, st style.Style) {
	if !b.InBounds(x, y) {
		return
	}
	i := b.idx(x, y)
...
func (b *Buffer) SetString(x, y int, s string, st style.Style) {
	...
	for i := 0; i < len(s); {
		if s[i] < utf8.RuneSelf {
			if col+1 > clip.X+clip.W {
				break
			}
			if col >= clip.X {
				b.setASCIICell(col, y, rune(s[i]), st)
			}
			col++
			i++
			continue
		...
		w := RuneWidth(r)
		if w == 0 {
			// Buffer cells cannot represent grapheme clusters, so retain unusual
			// zero-width code points conservatively as a normal cell.
			w = 1
		}
...
// DrawBorder paints a titled box border and fills its interior, the shared primitive behind layout.Bordered so bordered layout composition has one canonical panel surface.
...
func (b *Buffer) renderRegions(w io.Writer, regions []Rect, synchronized bool) (int, error) {
	...
	for _, rr := range regions {
		...
		for y := r.Y; y < r.Y+r.H; y++ {
			...
			rowBase := y * b.W
			x0, x1 := r.X, r.X+r.W-1
			// A damage rectangle can begin on the continuation half of a wide
			// glyph. Include its leading cell so the terminal never receives an
			// orphaned continuation update.
			if x0 > 0 {
				idx0 := y*b.W + x0
				if b.back[idx0].Ch == 0 && RuneWidth(b.back[idx0-1].Ch) == 2 {
					x0--
				}
			}
			if b.dirtyMin[y] > x0 {
			...
			for wi := ws; wi <= we; wi++ {
				...
				for word != 0 {
					...
					for i := idx; i < runEnd; i++ {
						c := b.back[i]
						// A zero rune immediately following a wide glyph is the
						// continuation cell. The terminal already advanced across it.
						if c.Ch == 0 && i > idx && RuneWidth(b.back[i-1].Ch) == 2 {
							continue
						}
