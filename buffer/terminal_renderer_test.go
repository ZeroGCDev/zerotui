...
func TestWideRuneOccupiesTwoCells(t *testing.T) {
	...
	st := style.New(color.White, color.Black)
	b.SetString(0, 0, "界A", st)
	if got := b.CellAt(0, 0).Ch; got != '界' {
		t.Fatalf("first glyph=%q", got)
	}
	if got := b.CellAt(1, 0).Ch; got != 0 {
		t.Fatalf("continuation=%q, want zero", got)
	}
	if got := b.CellAt(2, 0).Ch; got != 'A' {
		t.Fatalf("following glyph=%q", got)
	}
	var out bytes.Buffer
	if _, err := b.Render(&out); err != nil {
		t.Fatal(err)
	...
	if !bytes.Contains(out.Bytes(), []byte("界A")) {
		t.Fatalf("wide output=%q", out.String())
	}
}

func TestReplacingWideGlyphClearsContinuation(t *testing.T) {
	b := New(4, 1)
	st := style.New(color.White, color.Black)
	b.SetString(0, 0, "界A", st)
	b.Set(0, 0, 'X', st)
	if got := b.CellAt(1, 0).Ch; got != ' ' {
		t.Fatalf("stale wide continuation=%q, want space", got)
	}
}

func TestWritingIntoWideContinuationClearsLeadingGlyph(t *testing.T) {
	b := New(4, 1)
	st := style.New(color.White, color.Black)
	b.SetString(0, 0, "界A", st)
	b.Set(1, 0, 'X', st)
	if got := b.CellAt(0, 0).Ch; got != ' ' {
		t.Fatalf("stale wide leading glyph=%q, want space", got)
	}
}
