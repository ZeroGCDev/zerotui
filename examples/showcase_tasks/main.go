...
func main() {
	...
	go func() {
		...
		statuses := []string{"IN REVIEW", "IN PROGRESS", "DONE", "BLOCKED"}
		owners := []string{"Maya", "Arun", "Lena", "Noah"}
		for tick := 0; ; tick++ {
			time.Sleep(900 * time.Millisecond)
			rows := make([][]string, 4)
			for i := range rows {
				rows[i] = []string{[]string{"Landing page", "API docs", "Navigation", "Release notes"}[i], owners[rng.Intn(len(owners))], statuses[rng.Intn(len(statuses))]}
			}
			table.Rows = rows
			virtual.Selected = rng.Intn(200)
			a.BeginBatch()
			a.InvalidateWidgets(table, virtual)
			a.EndBatch()
			_ = tick
		}
	}()
