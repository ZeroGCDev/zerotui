...
func TestDecodeCtrlClipboardKeys(t *testing.T) {
	copyKey := decodeRune(0x03)
	if copyKey.Type != KeyCtrlC {
		t.Fatalf("Ctrl+C decoded as %+v", copyKey)
	}
	pasteKey := decodeRune(0x16)
	if pasteKey.Type != KeyRune || pasteKey.Rune != 'v' || pasteKey.Mods&ModCtrl == 0 {
		t.Fatalf("Ctrl+V decoded as %+v", pasteKey)
	}
}
