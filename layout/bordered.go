...
// Bordered draws a titled box around `child`'s entire computed layout, inset by one cell - the canonical panel container for a layout tree. Because it is a layout.Node like any other, every focusable widget inside `child` still flattens into the app's single top-level placement list, so Tab-navigation and mouse routing work exactly as if the box weren't there. focusedFn is optional; pass a func that reports whether any child widget currently has focus to auto-highlight the border.
...
func (b *bordered) PreferredHeight() int {
	if b == nil || b.child == nil {
		return 2
	}
	if ph, ok := b.child.(PreferredHeight); ok {
		h := ph.PreferredHeight() + 2
		if h < 2 {
			h = 2
		}
		return h
