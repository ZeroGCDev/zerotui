...
// AppendSignedFixedPrec appends a signed fixed-point integer without allocating.
func AppendSignedFixedPrec(dst []byte, v int64, decimals, show int) []byte {
	if v < 0 {
		dst = append(dst, '-')
		return AppendFixedPrec(dst, uint64(-(v+1))+1, decimals, show)
	}
	return AppendFixedPrec(dst, uint64(v), decimals, show)
}
