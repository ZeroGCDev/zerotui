...
// EnableRaw puts the terminal into raw mode, enters the alt screen, hides the hardware cursor and enables mouse reporting. zerotui renders widget cursors itself. It returns a restore func that must be called (typically via defer) before the process exits.
func EnableRaw() func() {
	run("stty", "raw", "-echo")
	EnterAltScreen()
	HideCursor()
...
func EnableMouse()           { os.Stdout.WriteString("\x1b[?1000h\x1b[?1002h\x1b[?1006h") }
func DisableMouse()          { os.Stdout.WriteString("\x1b[?1006l\x1b[?1002l\x1b[?1000l") }
func EnableBracketedPaste()  { os.Stdout.WriteString("\x1b[?2004h") }
func DisableBracketedPaste() { os.Stdout.WriteString("\x1b[?2004l") }
