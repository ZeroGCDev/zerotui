...
func NewOrderBook(decimals, sizeDecimals, showDecimals int) *OrderBook {
	return &OrderBook{Decimals: decimals, SizeDecimals: sizeDecimals, ShowDecimals: showDecimals, bids: make([]Level, 32), asks: make([]Level, 32), renderBids: make([]Level, 32), renderAsks: make([]Level, 32), dirtyFrom: 0, dirtyTo: 0, dirtyFull: true, bidMaxSize: 1, askMaxSize: 1}
}
...
func (o *OrderBook) Draw(buf *buffer.Buffer, area geometry.Rect, theme *style.Theme) {
	...
	o.mu.RLock()
	bidCount, askCount := o.bidCount, o.askCount
	if bidCount > len(o.renderBids) {
		o.mu.RUnlock()
		o.mu.Lock()
		if bidCount > len(o.renderBids) {
			o.renderBids = make([]Level, bidCount*2)
		}
		if askCount > len(o.renderAsks) {
			o.renderAsks = make([]Level, askCount*2)
		}
		o.mu.Unlock()
		o.mu.RLock()
	}
	...
	o.renderBids = o.renderBids[:bidCount]
	o.renderAsks = o.renderAsks[:askCount]
	copy(o.renderBids, o.bids[:bidCount])
	copy(o.renderAsks, o.asks[:askCount])
	bids := o.renderBids
	asks := o.renderAsks
