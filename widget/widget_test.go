...
func TestCellWidthMatchesTerminalGeometry(t *testing.T) {
	cases := []struct {
		name string
		text string
		want int
...
func TestEmptySelectableWidgetsNormalizeSelection(t *testing.T) {
	list := NewList(nil)
	list.Selected = -1
	list.move(1)
	if list.Selected != 0 {
		t.Fatalf("empty List selected=%d, want 0", list.Selected)
	}

	virtual := NewVirtualList(0, nil)
	virtual.Selected = -1
	virtual.HandleKey(input.Key{Type: input.KeyEnter})
	if virtual.Selected != 0 {
		t.Fatalf("empty VirtualList selected=%d, want 0", virtual.Selected)
	}
	...
	table.Selected = -1
	buf := buffer.New(10, 2)
	table.Draw(buf, geometry.Rect{W: 10, H: 2}, style.TokyoNightTheme())
	if table.Selected != 0 {
		t.Fatalf("empty Table selected=%d, want 0", table.Selected)
	}

	vt := NewVirtualTable(nil, 0, nil)
	vt.Selected = -1
	vt.Draw(buf, geometry.Rect{W: 10, H: 2}, style.TokyoNightTheme())
	if vt.Selected != 0 {
		t.Fatalf("empty VirtualTable selected=%d, want 0", vt.Selected)
	}
}

func TestTextInputWideMouseAndCursorUseCellColumns(t *testing.T) {
	in := NewTextInput("")
	in.SetValue("你a")
	in.Focus(true)
	buf := buffer.New(12, 1)
	in.Draw(buf, geometry.Rect{X: 0, Y: 0, W: 12, H: 1}, style.TokyoNightTheme())
	if got := buf.CellAt(2, 0).Ch; got != 'a' {
		t.Fatalf("wide glyph advanced cursor geometry incorrectly: cell 2 = %q, want 'a'", got)
	}
	in.HandleMouse(input.MouseEvent{Action: input.MousePress, X: 2, Y: 0}, geometry.Rect{X: 0, Y: 0, W: 12, H: 1})
	if in.cursor != 1 {
		t.Fatalf("mouse hit-testing cursor=%d, want 1", in.cursor)
	}
}
